...
if len(sys.argv) > 2:
    print(f"Usage: {sys.argv[0]} <Optional: use_array>")
    exit()
...
use_array = False
if len(sys.argv) > 1:
    use_array = sys.argv[1].lower() in ["t", "true", "array"]
    print(f"Arg received: {sys.argv[1]} / use array: {use_array}")
output_ext = "pdf"

log_dir = f"*{dataset}_{version}_coords{'_array' if use_array else ''}*"
log_files = glob.glob(f"{log_dir}/*/*_eval.json")
log_files = [x for x in log_files if "bg_aug" not in x and "rot_aug" not in x]
if not use_array:  # use array option will directly be enforced using the glob regex
    log_files = [x for x in log_files if "array" not in x]
print("Unfiltered file list:", len(log_files), log_files[:5])

log_files = [x for x in log_files if "views" not in x]
print("JSON output files:", len(log_files), log_files[:3])

output_dir = f"{dataset}_{version}_coords{'_array' if use_array else ''}_plots_{output_ext}/"
...
for file in log_files:
    ...
    assert file_name[1] == "coords"
    if use_array:
        offset = 1
        assert file_name[2] == "array"
