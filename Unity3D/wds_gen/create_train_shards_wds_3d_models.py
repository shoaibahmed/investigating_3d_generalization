...
def getChairsMultiRotWDS(root_dir, split, transform, train_stride=None, training_views=None, num_classes=None, training_axis="y", dataset_stride=36):
    """
    ...
    def filter_specific_training_views(sample):
        ...
        model_idx = int(paths[-3].replace("model_", ""))
        ...
        if split == "test":  # Select everything for the test set
            return model_idx < num_classes  # 0 indexed, so < condition
        ...
        if num_classes is not None:
            select_ex = select_ex and (model_idx < num_classes)  # 0 indexed, so < condition
    ...
    def preprocess(sample):
        ...
        return image, key, model_idx, rotation_axis, frame_idx, rot_x, rot_y, rot_z
...
version = "v6"
url = f"/netscratch/siddiqui/Datasets/Chairs_{version}.tar.xz"
dataset_stride = 1
output_dir = os.path.join(os.path.split(url)[0], f"Chairs_train_{version}")
...
class_list = [10, 50, 100, 500, 1000, 2500, 5000]

if create_test_set_files:
    for num_classes in class_list:
        # Create the dataset instance
        dataset = getChairsMultiRotWDS(url, split="test", transform=None, num_classes=num_classes)
        
        # Create the output file
        output_file = os.path.join(output_dir, f"Chairs_{version}_classes_{num_classes}_eval.tar.xz")
        ...
        total_ex = 0
        for idx, (image, key, model_idx, rotation_axis, frame_idx, rot_x, rot_y, rot_z) in enumerate(dataset):
            print(f"# classes= {num_classes} (# ex={idx}) // {key} // {model_idx} // {rotation_axis} // {frame_idx} // {rot_x}, {rot_y}, {rot_z}")
            sample = {
                "__key__": key,
                "image.jpg": image,
                "model_idx.cls": model_idx,
...
if create_equidistant_dataset:
    ...
    for num_views in [1, 2, 3, 4, 6, 12]:
        ...
        for num_classes in class_list:
            # Create the dataset instance
            dataset = getChairsMultiRotWDS(url, split="train", transform=None, train_stride=train_stide, dataset_stride=dataset_stride, num_classes=num_classes)
            
            # Create the output file
            output_file = os.path.join(output_dir, f"Chairs_{version}_classes_{num_classes}_equidistant_views_{num_views}.tar.xz")
            if os.path.exists(output_file):
                print(f"Warning: file already exists ({output_file}). Skipping file generation...")
                continue
            sink = wds.TarWriter(output_file, compress=True)
            ...
            total_ex = 0
            for idx, (image, key, model_idx, rotation_axis, frame_idx, rot_x, rot_y, rot_z) in enumerate(dataset):
                print(f"# views={num_views} (# ex={idx}) // {key} // {model_idx} // {rotation_axis} // {frame_idx} // {rot_x}, {rot_y}, {rot_z}")
                sample = {
                    "__key__": key,
                    "image.jpg": image,
                    "model_idx.cls": model_idx,
...
if create_limited_views_dataset:
    ...
    for view_list in ["0", "-30,30", "-30,0,30", "-30,-10,10,30", "-30,-15,0,15,30", "-30,-20,-10,0,10,20,30", "-30,-24,-18,-12,-6,0,6,12,18,24,30",
        ...
        for num_classes in class_list:
            ...
            dataset = getChairsMultiRotWDS(url, split="train", transform=None, training_views=training_views, dataset_stride=dataset_stride, num_classes=num_classes)

            # Create the output file
            output_file = os.path.join(output_dir, f"Chairs_{version}_classes_{num_classes}_selected_views_{view_list}.tar.xz")
            if os.path.exists(output_file):
                print(f"Warning: file already exists ({output_file}). Skipping file generation...")
                continue
            sink = wds.TarWriter(output_file, compress=True)
            ...
            total_ex = 0
            for idx, (image, key, model_idx, rotation_axis, frame_idx, rot_x, rot_y, rot_z) in enumerate(dataset):
                print(f"View list={view_list} (# ex={idx}) // {key} // {model_idx} // {rotation_axis} // {frame_idx} // {rot_x}, {rot_y}, {rot_z}")
                sample = {
                    "__key__": key,
                    "image.jpg": image,
                    "model_idx.cls": model_idx,
