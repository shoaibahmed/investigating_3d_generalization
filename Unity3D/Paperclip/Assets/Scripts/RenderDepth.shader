// Upgrade NOTE: replaced 'mul(UNITY_MATRIX_MVP,*)' with 'UnityObjectToClipPos(*)'

// Upgrade NOTE: replaced 'mul(UNITY_MATRIX_MVP,*)' with 'UnityObjectToClipPos(*)'

// Code adapted from: https://answers.unity.com/questions/877170/render-scene-depth-to-a-texture.html
// https://forum.unity.com/threads/adding-detph-to-channel-alpha.956193/


Shader "Custom/RenderDepth"
{
    Properties
    {
        _MainTex ("Base (RGB)", 2D) = "white" {}
        _DepthLevel ("Depth Level", Range(1, 3)) = 1
    }
    SubShader
    {
        UsePass "Legacy Shaders/VertexLit/SHADOWCASTER"

        Pass
        {
            CGPROGRAM

            #pragma vertex vert
            #pragma fragment frag
            #include "UnityCG.cginc"
            
            uniform sampler2D _MainTex;
            uniform sampler2D _CameraDepthTexture;
            uniform fixed _DepthLevel;
            uniform half4 _MainTex_TexelSize;

            struct input
            {
                float4 pos : POSITION;
                half2 uv : TEXCOORD0;
            };

            struct output
            {
                float4 pos : SV_POSITION;
                half2 uv : TEXCOORD0;
            };

            struct v2f {
                float4 pos : SV_POSITION;
                fixed4 color : COLOR;
                float2 depth : TEXCOORD0;
            };

            output vert(input i)
            {
                output o;
                o.pos = UnityObjectToClipPos(i.pos);
                o.uv = MultiplyUV(UNITY_MATRIX_TEXTURE0, i.uv);
                return o;
            }

            // output vert(input i)
            // {
            //     output o;
            //     o.pos = UnityObjectToClipPos(i.pos);
            //     o.uv = MultiplyUV(UNITY_MATRIX_TEXTURE0, i.uv);
            //     // why do we need this? cause sometimes the image I get is flipped. see: http://docs.unity3d.com/Manual/SL-PlatformDifferences.html
            //     #if UNITY_UV_STARTS_AT_TOP
            //     if (_MainTex_TexelSize.y < 0)
            //             o.uv.y = 1 - o.uv.y;
            //     #endif

            //     return o;
            // }
            
            // fixed4 frag(output o) : COLOR
            // {
            //     float depth = UNITY_SAMPLE_DEPTH(tex2D(_CameraDepthTexture, o.uv));
            //     depth = pow(Linear01Depth(depth), _DepthLevel);
            //     return depth;
            // }

            // fixed4 frag(output o) : COLOR
            // {
            //     float depth = UNITY_SAMPLE_DEPTH(tex2D(_CameraDepthTexture, o.uv));
            //     depth = pow(Linear01Depth(depth), _DepthLevel);
            //     return fixed4(depth, depth, depth, 1);
            // }

            fixed4 frag(output o) : COLOR
            {
                fixed4 color = tex2D(_MainTex, o.uv);
                float depth = UNITY_SAMPLE_DEPTH(tex2D(_CameraDepthTexture, o.uv));
                depth = pow(Linear01Depth(depth), _DepthLevel);
                return fixed4(color.rgb, depth);
            }

            // v2f vert(appdata_base v) {
            //     v2f o;
            //     o.pos = UnityObjectToClipPos(v.vertex);
            //     o.depth = -mul(_MainTex, v.vertex).z * _ProjectionParams.w;    // _ProjectionParams.w = 1/far
            //     return o;
            // }

            // half4 frag(v2f i) : COLOR {
            //     fixed invert = 1 - i.depth;
            //     return fixed4(invert, invert, invert, 1);
            // }
            
            ENDCG
        }
    } 
}
